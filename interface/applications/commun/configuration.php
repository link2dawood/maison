<?php
// D�marrage de la session (si pas d�j� active)
if (session_status() == PHP_SESSION_NONE) {
    session_start();
}

// En environnement de d�veloppement uniquement
...
// Connexion � la base de donn�es avec gestion d'erreurs am�lior�e
try {
    ...
    // D�finir le charset en UTF-8
    if (!$link->set_charset('utf8')) {
        throw new Exception('Erreur lors de la configuration du charset UTF-8');
    }
} catch (Exception $e) {
    // Log l'erreur (en production, �viter d'afficher les d�tails de connexion)
...
if ($result && $result->num_rows > 0) {
    ...
    $result->free(); // Lib�rer la m�moire du r�sultat
} else {
    // G�rer l'erreur si aucune configuration trouv�e
    error_log('Avertissement: Aucune configuration trouv�e dans la base de donn�es');
}

// Compl�ter avec des cha�nes vides si n�cessaire pour �viter les erreurs d'index
$array += array_fill(0, 34, '');

// D�finir les constantes critiques avec v�rification
...
// Fichiers inclus essentiels avec v�rification
...
// Constantes applicatives avec v�rification
...
// Fallback pour �viter erreurs avec v�rification
...
// D�finition des scripts JavaScript
if (!defined('CONFIGURATION_JS')) {
    define('CONFIGURATION_JS', '<script type="text/javascript" src="' . HTTP_HOST . '/interface/applications/commun/java.js"></script>' . "\n");
}
...
if (!defined('AFFICHER_EXTRAIT_FIN')) {
 ...
 if (!defined('PERIODE_CONNEXION')) {
    define('PERIODE_CONNEXION', $array[17]); // Dur�e de la connexion
}
...
if (!defined('RAFRAICHISSEMENT_PAGE')) {
    define('RAFRAICHISSEMENT_PAGE', $array[19]); // �ch�ance du rafra�chissement de la page
}
...
if (!defined('RAFRAICHISSEMENT_MESSENGER')) {
    define('RAFRAICHISSEMENT_MESSENGER', $array[21]); // �ch�ance du rafra�chissement de la page
}
...
if (!defined('NOMBRE_CARACTERES_EXTRAIT_DESCRIPTION')) {
    define('NOMBRE_CARACTERES_EXTRAIT_DESCRIPTION', $array[25]); // Nombre de caract�res extrait description
}
...
if (!defined('NOMBRE_MESSAGES_DUO_WEBCAM')) {
    define('NOMBRE_MESSAGES_DUO_WEBCAM', $array[27]); // Nombre de messages affich�s durant la conversation
}
if (!defined('RAFRAICHISSEMENT_MESSAGES_DUO_WEBCAMS')) {
    define('RAFRAICHISSEMENT_MESSAGES_DUO_WEBCAMS', $array[28]); // rafra�chissement toutes les X secondes
}
