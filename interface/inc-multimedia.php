						...
						<td style="text-align:center;">
							...
							if($video_existant){
								...
								if($inscription->en_ligne){
									echo '<p style="padding-top:25px;">'.fenetrePopUp(HTTP_SERVEUR.'interface/'.FILENAME_POPUP_IDENTITE.'?atn=9&action=rg&tpe='.$_GET['tpe'].'&ad='.$inscription->id.'',700,270,TEXTE_72).'</p>';
								}
								else{
					...
					<tr>
						<td style="text-align:center;">
							...
							if($audio_existant){
								//Regarder la vidéo
								if($inscription->en_ligne){
									echo '<p style="padding-top:25px;">'.fenetrePopUp(HTTP_SERVEUR.'interface/'.FILENAME_POPUP_IDENTITE.'?atn=10&action=ec&tpe='.$_GET['tpe'].'&ad='.$inscription->id.'',700,270,TEXTE_73).'</p>';
								}
								else{
									echo '<p style="padding-top:25px;">'.TEXTE_81.'</p>';
								}
