...
includeLanguage(RACINE, LANGUAGE, FILENAME_MOTEUR_RECHERCHE_PSEUDO);
	...
	if(empty($_SESSION['pseudo_client'])){
		...
		if(empty($_POST['search_pseudo'])){
			//RENVOI ACCUEIL
			redirection('0', HTTP_SERVEUR.'interface/'.FILENAME_MEMBRE_OFFLINE);
		}
		else{
			...
			if($result == 0 OR $compte_actif != 0){
				//ERREUR
				redirection('4', HTTP_SERVEUR.'interface/'.FILENAME_MEMBRE_OFFLINE);
				...
				redirection('4', HTTP_SERVEUR.'interface/'.FILENAME_PROFIL.'?id='.$pseudo_id.'&m='.$search_pseudo);
